...
from modules_eis.inputfile_handler import FileReader as mptFileReader

logger = get_logger('eis')


class FileReader(mptFileReader):
    ...
    def extract_and_calculate_missing_columns(
        ...
        if "|Z|/Ohm" not in df_calc or df_calc["|Z|/Ohm"].isna().any():
            if "|Z|/Ohm" not in df_calc:
                ...
                mask_mag = df_calc["|Z|/Ohm"].isna()
                df_calc.loc[mask_mag, "|Z|/Ohm"] = mag[mask_mag]

        if "Phase(Z)/deg" not in df_calc or df_calc["Phase(Z)/deg"].isna().any():
            if "Phase(Z)/deg" not in df_calc:
                df_calc["Phase(Z)/deg"] = phase
            else:
                mask_phase = df_calc["Phase(Z)/deg"].isna()
                df_calc.loc[mask_phase, "Phase(Z)/deg"] = phase[mask_phase]
